...
assessmentButton.addEventListener(
  ...
  () => { //アロー関数functionの代わりに使えます
    ...
    if(userName.length === 0 ){
      //名前がからっぽだったら処理終了
      return;
      //関数の途中にある return は、そこで関数を終了するという意味 値を返す場合は（）がいる
    }

    //診断結果表示エリアの作成（HTMLの要素を作る,h3タグとか）
    resultDivision.innerText = '' ;//実行前にresult-areaを空にする

    const header = document.createElement('h3'); //h3タグを作って
    header.innerText = '診断結果'; //タグ内のテキストを設定
    resultDivision.appendChild(header);//divタグの子要素として追加

    const paragraph = document.createElement('p');//pタグを作って
    const result = assessment(userName);//診断結果を作成
    paragraph.innerText = result;//pタグの内側テキスト設定
    resultDivision.appendChild(paragraph);//divタグの子要素に追加

    //Tweetエリアの作成
    tweetDivision.innerText = ''; //Tweetエリアのdivタグも空にする
